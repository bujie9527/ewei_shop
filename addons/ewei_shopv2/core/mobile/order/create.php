...
class Create_EweiShopV2Page extends MobileLoginPage
{
	...
	public function getcouponprice()
	{
		...
		$result = $this->caculatecoupon($contype, $couponid, $wxid, $wxcardid, $wxcode, $goodsarr, $goodsprice, $discountprice, $isdiscountprice, 1, '', '', $real_price);
		$goods_arr = $result['$goodsarr'];
		$goods_ids = array_column($goodsarr, 'goodsid', 'goodsid');
		$allGoods = pdo_fetchall('SELECT id,deduct,manydeduct,deduct2,manydeduct2 FROM ' . tablename('ewei_shop_goods') . ' WHERE id in (' . implode(',', $goods_ids) . ')', array(), 'id');
		$deductcredit = 0;
		$deductmoney = 0;
		$deductcredit2 = 0;
		$saleset = false;
		$saleset = m('common')->getPluginset('sale');
		$moneydeduct_status = false;
		$creditdeduct_status = false;
		$dispatch_deduct = false;
		...
		if (!(empty($saleset['creditdeduct']))) {
			$creditdeduct_status = 1;
		}


		if (!(empty($saleset['moneydeduct']))) {
			$moneydeduct_status = 1;

			if (!($saleset['dispatchnodeduct'])) {
				$dispatch_deduct = true;
			}
		...
		foreach ($result['$goodsarr'] as $v ) {
			$deductcredit_tmp = 0;
			$deductmoney_tmp = 0;
			$deductcredit2_tmp = 0;
			$goods_tmp = $allGoods[$v['goodsid']];

			if ($goods_tmp['status'] == 2) {
				continue;
			}


			if ($moneydeduct_status && (0 <= $goods_tmp['deduct2'])) {
				if ($goods_tmp['deduct2'] == 0) {
					$deductcredit2_tmp += $v['marketprice'];
				}
				 else if (0 < $goods_tmp['deduct2']) {
					$deductcredit2_tmp += $goods_tmp['deduct2'];
				}


				if ($goods_tmp['manydeduct2'] == 1) {
					$deductcredit2_tmp *= max(1, $v['total']);
				}


				$deductcredit2 += $deductcredit2_tmp;
			}


			if ($creditdeduct_status && (0 < $goods_tmp['deduct'])) {
				$deductmoney_tmp += $goods_tmp['deduct'];

				if ($goods_tmp['manydeduct'] == 1) {
					$deductmoney_tmp *= $v['total'];
				}


				$deductmoney += $deductmoney_tmp;
			}

		}

		if ($member['credit2'] < $deductcredit2) {
			$deductcredit2 = $member['credit2'];
		}


		if ($result['totalprice'] < $deductcredit2) {
			$deductcredit2 = $result['totalprice'];
		}


		if ($result['totalprice'] < $deductmoney) {
			$deductmoney = $result['totalprice'];
		}
		...
		if (0 < $saleset['money']) {
			$deductcredit = round(ceil($deductmoney / $saleset['money']), 2);

			if ($member['credit1'] < $deductcredit) {
				$deductcredit = $member['credit1'];
			}


			$deductmoney = round(ceil($deductcredit * $saleset['money']), 2);
		}
		 else {
			$total_credit = 0;
			$deductcredit = 0;
		...
		$result['deductcredit'] = $deductcredit;
		$result['deductmoney'] = floatval($deductmoney);
		$result['deductcredit2'] = floatval($deductcredit2);

		if (($result['totalprice'] - $result['discountprice'] - $result['deductprice'] - $result['isdiscountprice']) < $result['deductcredit2']) {
			$result['deductcredit2'] = $result['totalprice'] - $result['discountprice'] - $result['isdiscountprice'];
		}


		$result['include_dispatch'] = (($saleset['dispatchnodeduct'] == 1 ? 0 : 1));
		...
		if (is_array($goodsarr)) {
			...
			foreach ($goods as $k => $g ) {
				...
				case 1:
					...
					if (is_array($discountprice_array[$g['merchid']])) {
				...
				case 3:
					...
					if ($g['discounttype'] == 1) {
						...
						$discountprice *= 10000;
						$discountprice -= (double) $g['discountunitprice'] * (double) $g['total'] * 10000;

						if (0 < $discountprice) {
							$discountprice = price_format($discountprice / 10000);
						}
	...
	public function caculate()
	{
		...
		if (is_array($goodsarr)) {
			...
			if (!(empty($saleset))) {
				...
				if (!(empty($saleset['creditdeduct']))) {
					...
					if (($realprice - $seckill_payprice - $enough_tmp) < $deductmoney) {
						$deductmoney = $realprice - $seckill_payprice - $enough_tmp;
					}
		...
		$return_array['deductcredit2'] = floatval($deductcredit2);
	...
	public function submit()
	{
		...
		if (empty($goods[0]['bargain_id'])) {
			...
			if (!(empty($saleset['moneydeduct']))) {
				if (!(empty($_GPC['deduct2']))) {
					...
					if (($totalprice - $seckill_payprice - $dispatch_price) < $deductcredit2) {
						$deductcredit2 = $totalprice - $seckill_payprice - $dispatch_price;
					}


					if ($saleset['dispatchnodeduct'] < 1) {
		...
		$order['deductcredit2'] = floatval($deductcredit2);
	...
	public function getcardprice()
	{
		...
		$result = $this->caculatecard($card_id, $dispatch_price, $lotterydiscountprice, $taskcut, $goodsarr, $goodsprice, $discountprice, $isdiscountprice, $liveid, 1, $_GPC['goods_dispatch']);
	...
	public function caculatecard($card_id, $dispatch_price, $lotterydiscountprice, $taskcut, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $liveid, $isSubmit = 0, $goods_dispatch = array())
	{
		...
		if (!(empty($saleset))) {
			...
			if ($saleset) {
				foreach ($saleset['enoughs'] as $e ) {
					if ((floatval($e['enough']) <= $totalprice - $discountprice - $isdiscountprice) && (0 < floatval($e['money']))) {
						$deductenough_money = floatval($e['money']);
						$deductenough_enough = floatval($e['enough']);
		...
		$return_array['deductcredit2'] = (double) $deductcredit2;
		$return_array['deductcredit'] = $deductcredit;
		$return_array['deductmoney'] = $deductmoney;
		$return_array['$goodsarr'] = $goodsarr;
		return $return_array;
	...
	public function getcouponprice_tmp()
	{
		global $_W;
		global $_GPC;
		$openid = $_W['openid'];
