...
if (!(function_exists('pcUrl'))) {
	function pcUrl($do, $query = array(), $full = false)
	{
		global $_W;
		global $_GPC;
...
if (!(function_exists('arrayForceType'))) {
	/**
     * 数组强转数据类型
     ...
     * @return bool|array
     */
	function arrayForceType($arr = array(), $type = 'int', $implode = false)
	{
		if (is_string($arr)) {
			$arr = explode(',', $arr);
		}
...
function bar_code($strnum)
{
	...
	if ((14 < $length) && is_numeric($strnum)) {
		$b[17] = 'C';
		$j = 0;
		$key = 18;
		$ss = $length / 2;
		...
		while ($i < $ss) {
			...
			if ($iindex == 0) {
			 ...
			 else if (($b[($key + $i) - 1] == '0') && ($b[($key + $i) - 2] == '0') && ($chr != 'chr')) {
				$b[$key + $i] = '{';
				$b[$key + $i + 1] = 'C';
		...
		@$lastkey = end(array_keys($b));

		if (0 < ($length % 2)) {
			$lastnum = substr($strnum, -1);
		...
		@$b[15] = chr(end(array_keys($b)) - 15);
		$str = implode('', $b);
	}
	 else {
		$str = "\x1b" . 'd' . "\x02\x1d" . 'H2' . "\x1d" . 'hP' . "\x1d" . 'w' . "\x02\x1d" . 'kI' . chr($length + 2) . '{B' . $strnum;
	}
...
function parseTencentVideo($url)
{
	...
	if (strexists($url, 'v.qq')) {
		...
		$ret = redis()->setex(md5($tmp_url), 18000, $vurl);
		return $vurl;
	}
