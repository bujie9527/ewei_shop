...
class Pay_EweiShopV2Page extends AppMobilePage
{
	...
	public function main()
	{
		...
		pdo_update('ewei_shop_order', array('is_wxapp' => 1), array('id' => $order['id']));
		...
		if (empty($log)) {
			$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'ewei_shopv2', ':tid' => $order['ordersn'] . '_wxapp'));
		}
	...
	public function complete()
	{
		...
		if ($type == 'credit' || $type == 'cash') {
			...
			$tid = $order['ordersn'] . '_wxapp';
		}

		$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'ewei_shopv2', ':tid' => $tid));
		...
		if ($type == 'wechat') {
			...
			if (!is_error($payquery)) {
				...
				$ret['type'] = 'wechat';
				$ret['from'] = 'return';
				$ret['tid'] = str_replace('_wxapp', '', $log['tid']);
				$ret['user'] = $log['openid'];
				$ret['fee'] = $log['fee'];
				$ret['weid'] = $log['weid'];
				$ret['uniacid'] = $log['uniacid'];
	...
	protected function success($orderid)
	{
		...
		$uniacid = $_W['uniacid'];
		$member = m('member')->getMember($openid, true);
		$this->updateAgentId($uniacid, $openid, $_GPC['mid']);
...
';
	...
	public function updateAgentId($uniacid, $openid, $mid)
	{
		...
		$member_tmp = $memberModel->getMember($openid);

		if (intval($set['become_child']) == 2) {
			$parant_member = $memberModel->getMember($mid);
			$parent_is_agent = !empty($parant_member) && $parant_member['isagent'] == 1 && $parant_member['status'] == 1;
			if (empty($member_tmp['fixagentid']) && $parent_is_agent) {
				pdo_update('ewei_shop_member', array('agentid' => $parant_member['id'], 'childtime' => time(), 'fixagentid' => 1), array('uniacid' => $uniacid, 'id' => $member['id']));
			}
		}
	}
}
