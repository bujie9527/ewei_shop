...
class MemberController extends PluginMobilePage
{
    ...
    public function memberData($tag='member'){
        ...
        $data = array(
             'userinfo'=>$userinfo,
             'neworderlist'=>$this->getOrderList(),
//             'myfavorite'=>$this->getMyFavorite(),
//             'myhistory'=>$this->getMyHistory(),
            'ordercount'=>p('pc')->getOrderCount()
        );
    ...
    public function getOrderList(){
        ...
        $orders = pdo_fetchall("select o.price,o.ordersn,o.paytime,o.createtime,o.paytype,o.addressid,o.isverify,o.sendtype,o.status,o.id,og.goodsid,g.thumb from ".tablename('ewei_shop_order')." as o
				...
				where o.openid = :openid and o.deleted = 0 and o.status>2 and o.sendtime>0 and o.uniacid = :uniacid  GROUP by o.ordersn order by o.createtime desc limit 2 ",
    ...
    public function getMyFavorite(){
        ...
        if (!empty($total)) {
            $sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,g.merchid,g.minprice,g.maxprice FROM ' . tablename('ewei_shop_member_favorite') . ' f '
                ...
                . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT 3 ';
    ...
    protected function rf($type){
        ...
        if($_W['ispost']){
            ...
            if( !isset($_SESSION[$key]) ||  $_SESSION[$key]!==$verifycode || !isset($_SESSION['verifycodesendtime']) || $_SESSION['verifycodesendtime']+600<time() || empty(cache_load($key))){
        ...
        foreach ($id as $value){
            if(!empty($value)){
                $data = pdo_fetch("select id,goodsid,optionid, total from " . tablename('ewei_shop_member_cart') . " "
                    . " where id=:id and uniacid=:uniacid and openid=:openid limit 1 ", array(':id' => $value, ':uniacid' => $_W['uniacid'],':openid'=>$_W['openid']));
                if (!empty($data)) {
